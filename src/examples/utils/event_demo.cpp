...
public:
	...
	typedef q2b::event_source<MouseEventsListener*> MouseEventsSource;
...
private:
	...
	void onMouseMove(int x, int y) { 
		std::cout << "Mouse pointer moved to (" << x << ", " << y << ").\n";
	}
	
	void onMouseDown(int x, int y, int button) {
		std::cout << "Mouse button " << button << " pressed at (" << x << ", " << y << ").\n";
	}
	
	void onMouseUp(int x, int y, int button) {
		std::cout << "Mouse button " << button << " released at (" << x << ", " << y << ").\n";
	}
};
